...
3. **Gather live diagnostics (read-only)** — as available for the trigger:
   - `issue_id` non-null → `automake-db issue get <issue_id>` and `automake-db work list --id <issue_id>` (use `plugin_root`'s binary path; if it doesn't exist or errors, degrade to file-based inspection only and say so in Diagnosis).

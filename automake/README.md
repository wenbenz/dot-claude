...
Generated by `automake-db topology mermaid` against `automake/cli/automake-db/topology.default.json` — regenerate and re-paste here after editing the topology.
...
There is an eighth agent, `healer` (`automake/agents/healer.md`), that deliberately does **not** appear in the table above: it's cross-cutting rather than a per-state pipeline step. `start` invokes it out-of-band whenever it hits operational friction — a nonzero `automake-db` call, a missing required env var, a redundant build, an agent output plausibly caused by ambiguous wording in its own `.md`, or the user expressing dissatisfaction with pipeline output — and it reports back `FIXED` (only for low-risk `.md`/`topology.json`-value edits, applied on its own dedicated branch/worktree via the `create-pr` flow, never on the in-flight issue's branch), `PROPOSED` (any `.go` change, always shown as a diff, never applied), `ESCALATE`, or `NO_ACTION`. It never fires an `issue transition`, never appears in any state's `agents` list in the topology (so it can never perturb a round/retry limit), and never invokes `start` or itself. See `## Self-Healing` in `automake/skills/start/SKILL.md` for the full trigger list and invocation contract.

## State: `automake-db`

A Go CLI at `automake/cli/automake-db` (built on cobra, so flags and positional args may be interspersed in any order) is the only thing that writes to the database. It backs onto a single global, cross-repo SQLite file at `~/.claude/automake/state.db` (override with `$AUTOMAKE_DB`), so pipeline history and resumability work across every repo you use it in, not just one.

**Build it, then invoke the binary.** `start` builds it as its first step via `go install`, then calls it by bare command name, relying on `$PATH` to resolve it:
...
(`plan-tickets` still uses the older `go build -o <fixed path>` convention and invokes by that full path — it hasn't been migrated to `go install` yet, so both conventions currently coexist in this plugin.)

A pipeline run makes roughly 15 CLI calls. `go run` recompiles and relinks on each one — about 2.5s a call even with a warm build cache, against 0.03s for the built binary — so one build up front replaces some 40s of pure overhead. `go install` is itself cached, so re-running it when nothing has changed costs a couple of seconds. Unlike the old `go build -o` convention, `go install`'s destination (`$GOBIN`, or `$(go env GOPATH)/bin`, or `$HOME/go/bin`) is *not* inside the plugin's own content-addressed cache directory — it's a stable location outside it, which is what lets the bare `automake-db` name resolve via `$PATH` at all, but it also means a binary built from a previous plugin version can survive a plugin update undetected (see the caveat in `SKILL.md`'s `## The automake-db CLI`). The binary is self-contained: `schema.sql` and the default topology are both compiled in, so it does not need its source tree at runtime.

`start`'s setup step also drops a gitignored `.build_marker` file next to the source (`automake/cli/automake-db/.build_marker`) recording an identifier for what was built and when. It exists purely to detect a *redundant* build within one checkout — the setup block reporting `built` when a marker for that exact source path was already recorded — which would mean something removed the installed binary from `$PATH` that should still have been there; see `## Self-Healing` in `automake/skills/start/SKILL.md` for what happens when that fires.

**Schema** (see `automake/cli/automake-db/schema.sql` for the exact DDL, including indices):
...
Resolution order, as the CLI implements it: `--config PATH` flag → `$AUTOMAKE_TOPOLOGY` → the default compiled into the binary (`automake/cli/automake-db/topology.default.json`). There is deliberately no repo-root tier in the CLI — it stays repo-agnostic. A per-repo override lives at `<repo_root>/.claude/automake/topology.json`, and it is the orchestrating skill's job to notice one and pass it as `--config`, which means **a per-repo override outranks `$AUTOMAKE_TOPOLOGY`**, not the other way round.
...
- **`go`** — required to build `automake-db`. The skills build it themselves as a setup step, so there is nothing to install by hand. `start` uses `go install`, which needs its resolved destination (`$GOBIN`/`$(go env GOPATH)/bin`/`$HOME/go/bin`) to be writable and on `$PATH`; `plan-tickets` still uses `go build -o`, which needs the plugin directory itself writable instead. No cgo, no C toolchain needed.

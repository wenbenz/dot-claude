...
**Build it via `make`, then invoke the binary by bare name.** `start`'s setup step runs `automake/Makefile`'s `install` target as its first step, then calls the CLI by bare command name, relying on `$PATH` to resolve it:
...
(`plan-tickets` still uses the older `go build -o <fixed path>` convention and invokes by that full path — it hasn't been migrated to the Makefile yet, so both conventions currently coexist in this plugin.)

The `install` target builds `automake/bin/automake-db` (a sibling of `cli/`, so the compiled artifact stays inside the plugin's own content-addressed directory and is pinned to this exact plugin version) only when a source file, `go.mod`, or `go.sum` is newer than the existing binary — a plain `make` mtime comparison, with no `go` invocation at all on the other ~14 of the pipeline's ~15 calls per run once it's built once. It then unconditionally re-links `$GOBIN/automake-db` (or `$(go env GOPATH)/bin`, or `$HOME/go/bin`) to point at that binary, on every call — cheap (`ln -sf` is a single syscall), and it's what keeps a binary from a previous plugin version from lingering on `$PATH` after an update: the symlink always tracks whatever `$CLAUDE_PLUGIN_ROOT` resolves to on the current run, even on the calls where the underlying binary didn't need rebuilding. The binary is self-contained: `schema.sql` and the default topology are both compiled in, so it does not need its source tree at runtime.
...
- **`go`** — required to build `automake-db`. The skills build it themselves as a setup step, so there is nothing to install by hand. No cgo, no C toolchain needed.
- **`make`** — required by `start`'s setup step to build/install `automake-db` (`automake/Makefile`). The plugin directory needs to be writable (for `bin/automake-db`), and `automake-db`'s resolved `$GOBIN`/`$(go env GOPATH)/bin`/`$HOME/go/bin` needs to be writable and on `$PATH` (for the symlink `make install` creates there). `plan-tickets` still uses `go build -o` directly, without `make`, so it only needs the plugin directory writable.

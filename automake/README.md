...
`start`'s setup step also drops a gitignored `.build_marker` file alongside the binary (`automake/cli/automake-db/.build_marker`) recording the built binary's path and size/mtime after every successful build. It exists purely to detect a *redundant* build — the setup block reporting `built` when a marker for that exact binary path was already recorded — which would mean something removed a binary that should still have been there; see `## Self-Healing` in `automake/skills/start/SKILL.md` for what happens when that fires.

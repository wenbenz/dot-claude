...
func cmdIssueTransition(args []string) error {
	...
	// Guard the write on the status this transition was validated against. The
	// DB is shared across concurrent pipelines, so another process may have
	// transitioned this issue between the SELECT above and here; without the
	// guard we would apply a transition from a state the issue has already
	// left, which is exactly the drift the DFA exists to prevent.
	...
	if n == 0 {
		return fmt.Errorf("issue %d is no longer in state %q (changed concurrently); re-read it with 'issue get %d' and retry", id, status, id)
	}
	fmt.Println(next)
	return nil
}

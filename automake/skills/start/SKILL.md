...
```!
# check_redundant_build MARKER_PATH SOURCE_PATH
# Only called from the branch where a build is about to fire — i.e. `automake-db`
# was NOT already found on $PATH. SOURCE_PATH identifies *what's being built* (the
# plugin's cli/automake-db source directory), not where the resulting binary ends
# up — `go install`'s destination isn't known/fixed ahead of time. Returns 0
# (suspicious/redundant) only when a marker from an earlier successful build
# already exists AND names this exact source path; returns 1 (not redundant — the
# normal first-build case) when there is no marker yet, or it names a different
# path.
...
# record_build_marker MARKER_PATH SOURCE_PATH — call only after a successful build.
record_build_marker() {
  marker_path="$1"
  binary_path="$2"
...
SOURCE_DIR="${CLAUDE_PLUGIN_ROOT:-}/cli/automake-db"
...
elif command -v automake-db >/dev/null 2>&1; then
  echo "automake-db: already built ($(command -v automake-db))"
elif ! command -v go >/dev/null 2>&1; then
  echo "automake-db: BUILD FAILED — go not found on \$PATH — stop and show this to the user"
else
  if check_redundant_build "$MARKER" "$SOURCE_DIR"; then
    REDUNDANT_BUILD=1
  else
    REDUNDANT_BUILD=0
  fi
  if go install -C "$SOURCE_DIR" .; then
    if command -v automake-db >/dev/null 2>&1; then
      echo "automake-db: built ($(command -v automake-db))"
      record_build_marker "$MARKER" "$SOURCE_DIR"
      if [ "$REDUNDANT_BUILD" = "1" ]; then
        echo "automake-db: redundant build — a prior successful build for this exact source path was already recorded, but a rebuild fired anyway — invoke healer (trigger=redundant_build, issue_id=<current issue or null>, context={prior_marker: $MARKER, current_build: $SOURCE_DIR}) per ## Self-Healing; non-blocking, pipeline continues"
      fi
    else
      echo "automake-db: BUILD FAILED — go install succeeded but 'automake-db' is not resolvable on \$PATH afterward (its install destination — \$GOBIN, or \$(go env GOPATH)/bin, or \$HOME/go/bin — is presumably not on \$PATH) — stop and show this to the user"
    fi
  else
...
Every state change and every agent invocation goes through the `automake-db` binary, invoked by its bare command name — the setup block installs it via `go install` and relies on `$PATH` to resolve it from there on:

```
automake-db <args>
```

The setup block at the top of this skill builds that binary before anything else runs, but only if it isn't already there — the pipeline makes ~15 calls per run, and re-invoking `go install` on every single one costs about 2.5s a call for no benefit. Known caveat: unlike the plugin's own content-addressed cache directory, `go install`'s destination (`$GOBIN`, or `$(go env GOPATH)/bin`, or `$HOME/go/bin`) is stable across plugin updates, so a binary built from a previous plugin version can remain on `$PATH` and satisfy the already-built check above without getting rebuilt after an update — the redundant-build marker only detects a same-source rebuild within one checkout, not this cross-version staleness. If the build fails, **stop and show the user** — every step below depends on it. Do not fall back to `go run`.
...
```
automake-db work finish $run --output "<pr-agent's report>" --pr <PR URL>
```

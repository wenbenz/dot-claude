...
if [ -z "${CLAUDE_PLUGIN_ROOT:-}" ]; then
  ...
  echo "automake-db: BUILD FAILED — make not found on \$PATH — stop and show this to the user"
elif ! command -v go >/dev/null 2>&1; then
  echo "automake-db: BUILD FAILED — go not found on \$PATH — stop and show this to the user"
elif make -C "$CLAUDE_PLUGIN_ROOT" install; then
...
Every state change and every agent invocation goes through the `automake-db` binary, invoked by its bare command name — the setup block builds it via `automake/Makefile` and relies on `$PATH` to resolve it from there on:

```
automake-db <args>
```

The setup block at the top of this skill runs `make -C "$CLAUDE_PLUGIN_ROOT" install` before anything else, which builds `bin/automake-db` (a sibling of `cli/`, inside the plugin's own content-addressed directory, so the artifact itself is pinned to this exact plugin version) only when a source file is newer than the existing binary — a plain mtime comparison inside `make`, with no `go` invocation at all when nothing changed — then unconditionally re-links `$GOBIN/automake-db` (or `$(go env GOPATH)/bin`, or `$HOME/go/bin`) to point at it. That unconditional re-link is what keeps a stale binary from a previous plugin version from lingering on `$PATH` after an update: every one of the pipeline's ~15 calls per run re-points the symlink at whatever `$CLAUDE_PLUGIN_ROOT` resolves to on this run, even on the calls where the underlying binary didn't need rebuilding. If the build fails, **stop and show the user** — every step below depends on it. Do not fall back to `go run`.
...
`start` (and, for `user_dissatisfaction`, whichever agent is operating the conversation) invokes the `healer` agent (`automake/agents/healer.md`) out-of-band on four triggers. `healer` is **not** a topology state: it fires no `issue transition`, is never a declared agent for any state, and its invocation never replaces or delays the existing stop-and-show-user behavior above — it runs *in addition to* that behavior, not instead of it, and every existing happy-path step is otherwise unchanged.
...
(A prior version of this section had a fifth trigger, `redundant_build`, for detecting an unnecessary rebuild via a hand-rolled marker file. That whole detection problem is now `make`'s job — see `## The automake-db CLI` — so there is nothing left for `healer` to diagnose here.)
...
- Every invocation is non-blocking with respect to pipeline control flow: `healer` never changes `issues.status`, never re-routes the current step, and never itself pauses a step that wasn't already stopping for its own existing reason (`cli_error`/`missing_env_var` already stop the pipeline today — `healer` runs in addition to that stop, not as its cause; `bad_directive`/`user_dissatisfaction` never stopped the pipeline before and still don't).

...
Generated by `automake-db topology mermaid` against `automake/bin/automake-db/topology.default.json` — regenerate and re-paste after editing the topology, don't hand-edit this diagram.
...
4. **Check for a resumable issue** (skip if `ticket` is null — inline requests with no external reference always start fresh):
   ```
   "$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" issue list --ticket <ticket>
   ```
   If a result has a non-terminal `status` (not `done`/`failed`):
   - `issue_id` = its `id`; current status = its `status`
   - `"$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work list --id <issue_id>` → take the most recent row's `repo`, `branch`, `worktree`
   ...
   - **Resuming from `blocked`**: show the user why it was blocked (from the relevant agent's last report) and ask for clarification before calling `"$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" issue transition <issue_id> resume` and re-entering at Planner (step 1) with the clarification appended to the handoff.
   ...
   ```
   "$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" issue create --description "<one-line summary of the request>" --ticket <ticket or omit>
   ```
   `issue_id` = the printed id. Its status is now the topology's initial state (`queued`); immediately:
   ```
   "$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" issue transition <issue_id> start
   ```
...
```
```
run=$("$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work start --id <issue_id> --agent planner --repo <repo_root> --branch <branch_name> --worktree <worktree_path> --context <pipeline_dir>/handoff_planner.json)
```
Call `planner` with `<pipeline_dir>/handoff_planner.json`. Write output to `<pipeline_dir>/plan.md`.
```
"$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work finish --output <pipeline_dir>/plan.md $run
```

- Output is a normal requirements+architecture doc → `"$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" issue transition <issue_id> success` → step 2.
...
```
coder_run=$("$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work start --id <issue_id> --agent coder --repo <repo_root> --branch <branch_name> --worktree <worktree_path> --context <pipeline_dir>/handoff_coder.json)
testwriter_run=$("$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work start --id <issue_id> --agent test-writer --repo <repo_root> --branch <branch_name> --worktree <worktree_path> --context <pipeline_dir>/handoff_test_writer.json)
```
...
```
"$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work finish --output "<coder's Files Written/Modified summary>" $coder_run
"$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work finish --output "<test-writer's Test Files Written summary>" $testwriter_run
```

Once **both** finish (join point — `coding` is one state covering both agents):
```
"$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" issue transition <issue_id> success
```
...
```
```
run=$("$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work start --id <issue_id> --agent validator --repo <repo_root> --branch <branch_name> --worktree <worktree_path> --context <pipeline_dir>/handoff_validator.json)
```
Call `validator`; write output to `<pipeline_dir>/validator_report.md`.
```
"$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work finish --output <pipeline_dir>/validator_report.md $run
```
...
```
```
run=$("$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work start --id <issue_id> --agent reviewer --repo <repo_root> --branch <branch_name> --worktree <worktree_path> --context <pipeline_dir>/handoff_reviewer.json)
```
Call `reviewer`; write output to `<pipeline_dir>/reviewer_report.md`.
```
"$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work finish --output <pipeline_dir>/reviewer_report.md $run
```
...
```
```
run=$("$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work start --id <issue_id> --agent doc-patcher --repo <repo_root> --branch <branch_name> --worktree <worktree_path> --context <pipeline_dir>/handoff_doc_patcher.json)
```
Call `doc-patcher`; save updated file list as `doc_files`.
```
"$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work finish --output "<Docs Updated summary>" $run
"$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" issue transition <issue_id> success
```
→ step 6.
...
```
```
run=$("$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work start --id <issue_id> --agent pr-agent --repo <repo_root> --branch <branch_name> --worktree <worktree_path> --context <pipeline_dir>/handoff_pr_agent.json)
```
...
```
"$CLAUDE_PLUGIN_ROOT/bin/automake-db/automake-db" work finish $run --output "<pr-agent's report>" --pr <PR URL>
```

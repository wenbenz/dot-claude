...
Create a new pipeline agent following Claude Code conventions. Agents are skills that run as isolated subagents (`context: fork`) with a focused, single-purpose role.
...
| Isolation | Runs in main context by default | Always `context: fork` |
...
| User-invocable | Yes | Usually `false` |
